#!/usr/bin/env python3
"""Validate and serial-flash the signed M13 application with matching artifacts."""
...
def load_flash_command(
    ...
    if before != "default-reset" or after != "hard-reset":
        fail("generated reset behavior differs from the reviewed M13 serial workflow")
    ...
    if app.get("encrypted") != "false":
        fail("M13 serial helper does not support an encrypted application entry")
    for required_section in ("bootloader", "partition-table", "otadata"):
        ...
        if section.get("encrypted") != "false":
            fail(f"M13 serial helper does not support encrypted {required_section} data")
    ...
    for section_name, expected_offset in expected_offsets.items():
        ...
        if actual_offset != expected_offset:
            fail(
                ...
                f"the generated M13 layout at 0x{expected_offset:x}"
            )
    actual_offsets = {
        normalized_offset(offset, "flash-file offset") for offset in flash_files
    }
    if actual_offsets != set(expected_offsets.values()):
        fail("generated flash map contains missing or unexpected M13 write offsets")
    ...
    if write_arguments not in permitted_write_arguments:
        fail("generated write-flash options differ from the reviewed M13 configuration")
...
def validate_artifacts(
    ...
    if failures:
        fail("effective M13 sdkconfig is invalid:\n  - " + "\n  - ".join(failures))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate and flash the signed M13 image with the bootloader, partition "
    ...
    try:
        ...
        print(f"Signed M13 serial validation failed: {error}", file=sys.stderr)
        return 1

    print("Validated signed M13 serial image set:")
    for offset, image in flash_map:
        print(f"  {offset}: {image}")
    if arguments.check_only:
        print("Signed M13 serial preflight: PASS (no board was written)")
        return 0
    ...
    print("Signed M13 serial flash: PASS")
    return 0
